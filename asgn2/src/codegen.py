...
class codeGenerator():
    ...
    def __init__(self,symTab,threeAC,varAllocate, code):
    ...
    def StatementType (self, line):
        # binary arithmetic
        if (line[1] in op32_dict):
            if (isinstance(line[3], int) and isinstance(line[4], int)):
                return ('BA_2C')
            elif (!isinstance(line[3], int) and isinstance(line[4], int)):
                return ('BA_1C_R')
            elif (isinstance(line[3], int) and !isinstance(line[4], int)):
                return ('BA_1C_L')
            else
    ...
    def handle_binary (self, lineno, op, lhs, op1, op2):
        ...
        loc, msg = self.varAllocate.getReg() # Need to send things from here. What though?

        line = self.code[lineno]
        op = self.op32_dict[line[1]]
        lineno, operator, lhs, op1, op2 = line
        statTyp = StatementType(line)
        ascode = ""

        if self.symbolToRegister[op1.name] is not None:
                loc_op1 = self.symbolToRegister[op1.name] # Fetching register, which is prefered if it exists
            else:
                loc_op1 = self.getFromMem(op1.name)
        if self.symbolToRegister[op2.name] is not None:
                loc_op2 = self.symbolToRegister[op2.name]
            else:
                loc_op2 = self.getFromMem(op2.name)

        if (statTyp == 'BA_2C'):
            reg = self.varAllocate.getReg(blockIndex, line) 
            ascode =  op + " $" + op1 + "," + loc + "\n" + "add $" + op2 + "," + loc + "\n"
            ...
            elif (msg == "Replaced NetUse"):
                
            elif (msg == "Replaced nothing"):
                ascode = op + " " + loc_op1 + "," + getFromMem(lhs.name) + "\n" + op + " " + loc_op2 + "," + getFromMem(lhs.name) "\n"
